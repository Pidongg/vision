...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(
        ...
        if getattr(self, "skip_nms", False):
            results = []
            for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
                # Clip boxes to image boundaries
                boxes = box_ops.clip_boxes_to_image(boxes, image_shape)
                
                # For each proposal, use the box regression from the highest scoring non-background class
                scores_no_bg = scores[:, 1:]  # [N, num_classes-1]
                best_class = torch.argmax(scores_no_bg, dim=1) + 1  # +1 because we removed background
                box_idx = torch.arange(boxes.size(0), device=device)
                
                # Select the best box for each proposal
                boxes_selected = boxes[box_idx, best_class]  # Shape: [N, 4]
                ...
                result = torch.cat([boxes_out, scores_no_bg], dim=1)
            ...
            for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
                ...
                # Store the original indices and class indices for mapping back
                box_indices = torch.arange(len(scores_dist_full), device=device).repeat_interleave(num_classes - 1)
            
                # Filter out low scoring boxes
                inds = torch.where(scores > self.score_thresh)[0]
                boxes, scores, labels = boxes[inds], scores[inds], labels[inds]
                box_indices = box_indices[inds]

                # remove empty boxes
                keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)
                boxes, scores, labels = boxes[keep], scores[keep], labels[keep]
                box_indices = box_indices[keep]
                ...
                if boxes.shape[0] > 0:
                    ...
                    # Get the full probability distributions for the kept boxes
                    scores_dist = scores_dist_full[box_indices]
                else:
                    scores_dist = scores_dist_full.new_zeros((0, num_classes))

                all_boxes.append(boxes)
                all_scores.append(scores)
                all_labels.append(labels)
                all_scores_dist.append(scores_dist)  # This now contains probabilities for all classes

            return all_boxes, all_scores, all_labels, all_scores_dist
    ...
    def forward(
        ...
        if self.training:
            ...
            boxes, scores, labels, scores_dist = self.postprocess_detections(class_logits, box_regression, proposals, image_shapes)
...
class RoIHeadsWithScores(RoIHeads):
    """
    Extension of RoIHeads that also returns the full distribution of class scores
    for each detection, excluding background class.
    """
    ...
    def __init__(
        ...
        self.class_conf_thresh = class_conf_thresh
    ...
    def postprocess_detections(
        ...
        if self.skip_nms:
            ...
            for boxes, scores, image_shape in zip(pred_boxes_list, pred_scores_list, image_shapes):
                ...
                # Filter out low scoring boxes using class-specific thresholds
                if self.class_conf_thresh is not None:
                    # Create a tensor with appropriate threshold for each detection
                    if isinstance(self.class_conf_thresh, dict):
                        # If dictionary mapping class indices to thresholds
                        thresholds = torch.ones_like(scores) * self.score_thresh
                        for cls_idx, thresh in self.class_conf_thresh.items():
                            cls_mask = (labels == cls_idx)
                            thresholds[cls_mask] = thresh
                    elif isinstance(self.class_conf_thresh, (list, tuple, torch.Tensor)):
                        # If list/tensor with threshold per class
                        thresholds = torch.tensor([self.class_conf_thresh[l] if l < len(self.class_conf_thresh) 
                                                   else self.score_thresh for l in labels], 
                                                  device=scores.device, dtype=scores.dtype)
                
                    # Apply class-specific thresholds
                    inds = torch.where(scores > thresholds)[0]
                else:
